#!/usr/bin/env python3
"""Batch docking sampler using PeSTo pocket centers.

This script runs in two steps:
1) Read meta/pocket_meta_all.csv, filter rows with fnat < 0.1, generate one yml per row.
...
def load_selected_rows(meta_csv_path: Path, fnat_threshold: float = 0.1) -> List[Dict[str, object]]:
    """Load rows where fnat < threshold and pocket_center is valid."""
    ...
    with meta_csv_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            pocket_idx = str(row.get("pocket_idx", "")).strip()
            fnat = _parse_float(str(row.get("fnat", "")))
            pocket_center = parse_pocket_center(str(row.get("pocket_center", "")))
            if not data_id or not pocket_idx:
                continue
            if fnat is None or fnat >= fnat_threshold:
                continue
...
def build_config_from_row(
    ...
    pocmol_args["data_id"] = f"dockpep_{data_id}_{pocket_idx}"
...
def generate_configs(
    ...
    for row in selected_rows:
        ...
        cfg_name = f"dock_pep_{data_id}_{pocket_idx}.yml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate yml from PeSTo pocket csv and run PocketXMol docking."
    )
    parser.add_argument(
        "--input_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Output root; docking outputs go to output_dir/pesto_pocket/output.",
    )
    parser.add_argument(
        "--template",
        type=Path,
...
def main() -> None:
    ...
    meta_csv_path = input_dir / "meta" / "pocket_meta_all.csv"
    cfg_dir = input_dir / "configs" / "pesto_pocket"
    dock_output_dir = output_dir / "pesto_pocket" / "output"
    ...
    if existing_cfg:
        ...
        failures: List[str] = []
        print("\n=== Step 1/2: Generate yml files from pocket_meta_all.csv ===")
        print(
            f"[Step1] SKIP: reuse {len(cfg_paths)} existing yml under {cfg_dir} "
            "(skip generation; Step 2 order by pep_len from each config)"
        )
    else:
        selected_rows = load_selected_rows(meta_csv_path=meta_csv_path, fnat_threshold=0.1)
        if not selected_rows:
            print(f"No valid rows with fnat < 0.1 found in: {meta_csv_path}")
            return

        n_pesto = len(selected_rows)
        selected_rows, n_rm = filter_rows_by_pass_peptide_check(
            selected_rows, input_dir, id_keys=("data_id",)
        )
        if n_rm:
            print(
                f"[Step1] peptide_all_check filter: removed {n_rm} / {n_pesto} rows "
                f"(pass_all_checks & pep_len<25 only; by data_id)"
            )
        if not selected_rows:
            print(
    ...
    step2_stats = run_sampling_loop(
        ...
        run_time_file=output_dir / "pesto_pocket" / "run_time_pesto_pocket.txt",
        failure_log_file=output_dir / "pesto_pocket" / "failure_log_pesto_pocket.txt",
        log_prefix="[Step2]",
    )
